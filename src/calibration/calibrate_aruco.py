...
def make_detect_fn():
    ...
    detect = lambda gray: det.detectMarkers(gray)[:2]
...
def open_camera(cfg, index):
    grabber = PseyepyGrabber(cfg, index)
...
def solve_pose(obj_pts, img_pts, K, dist):
    ...
    # ratio of second-best to best error; near 1 means the pose is ambiguous
...
def camera_center(rvec, tvec):
    ...
    center = (-R.T @ tvec.reshape(3, 1)).ravel()
        ...
        while len(samples) < SAMPLES:
            ...
            if ids is not None:
                ...
                if len(idx):
                    ...
                    # discard the first WARMUP_FRAMES so auto-exposure has settled
...
def main():
    """
    Locates every configured camera against the ArUco marker and writes its
    extrinsics back into the camera config.
    """
    ...
    for name, index in zip(names, indices):
        ...
        locate_camera(path, detect, index)
